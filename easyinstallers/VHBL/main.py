...
def writeAcc(acc):
    global account
    account = acc
    import chooseBackupVHBL
    chooseBackupVHBL.pushVars(account)
    close_window(root)
    chooseBackupVHBL.vp_start_gui()
...
class Account_Selector:
    def __init__(self, top=None):
        ...
        self.Button1.configure(command = lambda: writeAcc(self.accountList.get(ACTIVE)))
