global sqlQuery
sqlQuery = """CREATE TRIGGER ALLOW_URIS AFTER INSERT ON tbl_uri
...
def patch(backup):
    ...
    if not os.path.exists(CMA + "/EXTRACTED/APP/URICALL00"):
        shutil.copytree(defs.getWorkingDir()+"/easyinstallers/UriCaller/URICALL00" , CMA + "/EXTRACTED/APP/URICALL00")
    sign_support.goSign(account, "APP", "URICALL00",True)
    tkMessageBox.showinfo(title="UriCaller",message="Completed! Refresh QCMA Database then restore the SYSTEM Backup + The URICaller App.")
...
class Unsign_Backup:
    def __init__(self, top=None):
        ...
        self.Button1.configure(text='''Patch Uri Calls''')
